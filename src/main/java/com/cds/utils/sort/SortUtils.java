...
public class SortUtils {
    ...
    public static final int N = 212_0000;
...
class SortTask extends RecursiveAction {
    ...
    private int partition(int[] array, int start, int end) {
        ...
        int j = end;
        if (j - i > 2) {
            if ((array[i] < array[j - i] && array[j - i] < array[j]) || (array[j] < array[j - i]
                    && array[j - i] < array[i])) {
                int t = array[i];
                array[i] = array[j - i];
                array[j - i] = t;
            } else {
                if ((array[i] < array[j] && array[j] < array[j - i]) || (array[j - i] < array[j]
                        && array[j] < array[i])) {
                    int t = array[i];
                    array[i] = array[j];
                    array[j] = t;
