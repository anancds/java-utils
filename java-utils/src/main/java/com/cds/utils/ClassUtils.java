...
/**
 * Created by cds on 11/20/16 17:55.
 * utils of reflection
 */
public class ClassUtils {
     ...
     * @param extendField 是否获取接口或者父类中的公共属性
     * @return 属性字段数组
     */
    public final static String[] getField(String className, boolean extendField) {
        ...
        if (extendField) {
            Field[] fieldsz = theClass.getDeclaredFields();
            if (fieldsz != null) {
                for (Field field : fieldsz) {
                    set.add(field.getName());
                }
            }
        }
     ...
     * @param extendsField 是否获取接口或者父类中的公共属性
     * @return 属性字段数组
     */
